# Coconut Grove

**Code:** CGR
**Full name:** Coconut Grove
**City:** Kailua-Kona, Hawaii *(Emma wrote "Kaile Kona," read as Kailua-Kona. Confirm spelling before it goes on a letter.)*
**Street address:** *(unknown)*
**Property type:** *(unknown, owner entity name suggests retail)*

*Source: Emma, 2026-09-08, "Coconut Grove Retail XIII , LLC - Coconut Grove - Kaile Kona Hawaii"*

## Owner
- **Current owner: Coconut Grove Retail XIII, LLC**
- Owner contact name: *(unknown)*
- Phone: *(unknown)*
- Email: *(unknown)*

## Key contacts
*(none recorded yet)*

## Vendors

| Trade | Company | Contact | Notes |
|-------|---------|---------|-------|
| *(none recorded yet)* | | | |

## Lease quirks
*(none recorded yet)*

## The thing to remember
This is the only property outside Arizona.

**Hawaii is 3 hours behind Arizona, all year.** Neither state observes daylight saving, so the gap never changes. 9:00 AM in Scottsdale is 6:00 AM in Kona. Do not schedule a call or expect a same-morning reply on Hawaii time without checking the clock first. (Time zone math from CC, not a fact about the property.)

The owner entity carries a Roman numeral, XIII. Copy it exactly. Do not write 13.

---
*Card started 2026-09-08. Blank fields mean unknown, not empty. CC fills these in as real facts come up, never by guessing.*
